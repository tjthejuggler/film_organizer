...
export async function refreshDriveQueue() {
  ...
  $("#queueList").innerHTML = groups.length ? groups.map(g => `
    ...
    ${g.items.map(it => {
    // moves wait for their destination (plus source drives / favorites'
    // liked drive), deletes for payload.drives — `missing` names exactly
    // what to plug in
    const itemDrives = it.drives || [g.drive];
    const missing = it.missing || [];
    const wait = it.status === "running" || it.ready ? "" : missing.length
    ...
    // a RUNNING entry is shown as busy: the runner is single-lane, so
    // this also explains why later entries have not started yet
    const runBadge = '<span class="badge warn" title="This entry is moving right now">▶ running…</span>';
    return `
  <li class="qitem">
    ...
    ${it.status === "running" ? "" : `<button class="btn mini ghost" data-cancel-queue="${it.id}" title="Remove from queue">✕</button>`}
  </li>`;}).join("")}`).join("")
